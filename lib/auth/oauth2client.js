...
var OAuth2Client = (function (_super) {
    ...
    OAuth2Client.prototype.refreshToken = function (refresh_token, callback) {
        ...
        return this.transporter.request(uri, {
            contentType: 'form',
            data: {
    ...
    OAuth2Client.prototype.revokeToken = function (token, callback) {
        var url = OAuth2Client.GOOGLE_OAUTH2_REVOKE_URL_ + '?' +
            querystring.stringify({ token: token });
        this.transporter.request(url).then(function(res) {
          callback(null, res.json, res);
        }, callback);
    };
    /**
     ...
     */
    OAuth2Client.prototype.request = function (url, opts) {
        ...
        authCb = function (err, headers, response) {
            if (err) {
                ...
                return _this.transporter.request(url, opts).then(function(res) {
                    postRequestCb(null, res.json, res);
                }).catch(postRequestCb);
            }
        };
    ...
    OAuth2Client.prototype.getFederatedSignonCerts = function (callback) {
        ...
        var url = OAuth2Client.GOOGLE_OAUTH2_FEDERATED_SIGNON_CERTS_URL_;
        this.transporter.request(url).then(function(res) {
            ...
            _this._certificateCache = res.json;
            callback(null, res.json, res);
        }).catch(function(error) {
          callback(new transporters_1.RequestError('Failed to retrieve verification certificates: ' + err));
        });
    };
    /**
